...
import { getDatabase } from 'firebase-admin/database'; // تأكد من هذه الاستيرادات



// التهيئة المبسطة (بدون async)
...
// التهيئة الآمنة (تتحقق من عدم وجود تطبيق مثبت مسبقاً)
if (getApps().length === 0) {
  initAdminApp();
}
// تعريف db في النطاق العام
const db = getDatabase(); // هذا السطر أساسي
...
export const api = onRequest({ cors: true }, async (req, res) => {
    ...
    res.json({ status: 'يعمل الآن بدون أخطاء!' });
  } catch (error) {
...
// الدوال المساعدة (يجب تحويلها لـ ES Modules)
...
async function setAlert(data) {
  ...
  await db.collection('alerts').doc(id).set(alertData);
