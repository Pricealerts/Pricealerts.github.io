...
const EXCHANGES_CONFIG = {
	...
	bybit: {
		...
		candlestickUrl: "https://api.bybit.com/v2/public/kline/list",
		usdtSuffix: "USDT",
		parseCandle: c => ({
			time: parseInt(c.open_time) * 1000,
			open: parseFloat(c.open),
	...
	bitget: {
		...
		candlestickUrl: "https://api.bitget.com/api/spot/v1/market/candles",
	...
	for (let i = data.length - 1; i >= 0; i--) {
		...
		if (status === "Active") {
			...
			let timeDifferenceMs =
				currentTriggerTime.getTime() - requestTime.getTime();
			...
			let candles = await fetchCandlestickData(
				exchangeId,
				symbol,
				interval,
				limit
...
async function fetchCandlestickData(exchangeId, symbol, interval, limit) {
	...
	try {
		...
		switch (exchangeId) {
			...
			case "coinbase":
				apiUrl =
					exchange.candlestickUrl.replace(
						"{symbol}",
						`${symbol}${exchange.usdtSuffix}`
		...
		console.error(
			`خطأ في جلب بيانات الشموع لـ ${symbol} من ${exchange.name}:`,
			error
		);
		return null;
	}
