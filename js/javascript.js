document.addEventListener('DOMContentLoaded', () => {
    ...
    const conditionLessThanOrEqual = document.getElementById('conditionLessThanOrEqual');
    const conditionGreaterThanOrEqual = document.getElementById('conditionGreaterThanOrEqual');
    ...
    const telegramChatIdContainer = document.getElementById('telegramChatIdContainer');
    ...
    const APPS_SCRIPT_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbxc9Hk-vv-jJIZFzdAHUBwTxK2eNR44AnM2ExFPQGb8TrqSCyTxLpCeFC4LJ19v7hyf/exec";
    ...
    const EXCHANGES = {
        ...
        okx: {
            name: "OKX",
            exchangeInfoUrl: 'https://www.okx.com/api/v5/public/instruments?instType=SPOT',
...
function renderAlerts(alerts) {
  ...
  alerts.forEach((alert) => {
    ...
    function renderAlerts(alerts) {
        alertsList.innerHTML = '';
        if (!alerts || alerts.length === 0) {
            alertsList.innerHTML = '<li class="no-alerts-message">لا توجد تنبيهات نشطة حاليًا.</li>';
            return;
        }
    ...
    async function fetchTradingPairs(exchangeId) {
        ...
        if (!exchange) {
            ...
            currencySelect.innerHTML = '<option value="">الرجاء اختيار منصة صحيحة</option>';
            return;
        }
        ...
        try {
            ...
            switch (exchangeId) {
                ...
                case 'kucoin':
                    ...
                    if (data.code === '200000' && data.data) {
                        symbols = data.data
                            .filter(s => s.symbol.endsWith(exchange.usdtSuffix) && s.enableTrading)
                            .map(s => s.symbol);
                    } else {
                        console.error(`خطأ من KuCoin API (exchangeInfo):`, data.msg || JSON.stringify(data));
                    }
                    break;
                case 'bybit':
                    response = await fetch(exchange.exchangeInfoUrl);
                    ...
                    if (data.ret_code === 0 && data.result) {
                        symbols = data.result
                            .filter(s => s.quote_currency === exchange.usdtSuffix && s.status === 'Trading')
                            .map(s => s.base_currency + exchange.usdtSuffix); 
                    } else {
                        console.error(`خطأ من Bybit API (exchangeInfo):`, data.ret_msg || JSON.stringify(data));
                    }
                    break;
                case 'okx':
                    response = await fetch(exchange.exchangeInfoUrl);
                    ...
                    if (data.code === '0' && data.data) {
                        symbols = data.data
                            .filter(s => s.instType === 'SPOT' && s.quoteCcy === exchange.usdtSuffix.replace('-', '') && s.state === 'live')
                            .map(s => s.instId); 
                    } else {
                        console.error(`خطأ من OKX API (exchangeInfo):`, data.msg || JSON.stringify(data));
                    }
                    break;
                default:
            ...
            if (symbols.length > 0) {
                ...
                currencySelect.innerHTML = '<option value="">لا توجد أزواج USDT متاحة</option>';
            ...
            switch (exchangeId) {
                ...
                case 'kucoin':
                    ...
                    if (data.code === '200000' && data.data && data.data.price) {
                        ...
                        console.error(`خطأ من KuCoin API (ticker):`, data.msg || JSON.stringify(data));
                    }
                    break;
                case 'bybit':
                    apiUrl = `${exchange.tickerPriceUrl}?symbol=${symbol}`;
                    ...
                    if (data.ret_code === 0 && data.result && data.result.length > 0) {
                        ...
                        console.error(`خطأ من Bybit API (ticker):`, data.ret_msg || JSON.stringify(data));
                    }
                    break;
                case 'okx':
                    ...
                    response = await fetch(apiUrl);
                    data = await response.json();
                    if (data.code === '0' && data.data && data.data.length > 0 && data.data[0].last) {
                        price = parseFloat(data.data[0].last);
                    } else {
                        console.error(`خطأ من OKX API (ticker):`, data.msg || JSON.stringify(data));
                    }
                    break;
                default:
    ...
    function startPriceUpdates() {
        ...
        if (selectedSymbol && currentExchangeId) {
            ...
            priceUpdateInterval = setInterval(() => fetchCurrentPrice(currentExchangeId, selectedSymbol), 5000);
        } else {
    ...
    function checkForBrowserAlerts() {
        ...
        activeBrowserAlerts.filter(alert => alert.status === 'Active').forEach(alert => {
            if (alert.symbol === selectedSymbol && alert.exchangeId === currentExchangeId) {
                let shouldTrigger = false;
                if (alert.alertCondition === 'less_than_or_equal' && currentPrice <= alert.targetPrice) {
                    shouldTrigger = true;
                } else if (alert.alertCondition === 'greater_than_or_equal' && currentPrice >= alert.targetPrice) {
                    shouldTrigger = true;
                }

                if (shouldTrigger) {
                    showBrowserNotification(alert.symbol, currentPrice, alert.targetPrice, alert.alertCondition);
    ...
    async function manageAlertOnAppsScript(action, alertData = null) {
        alertStatus.textContent = `جاري ${action === 'setAlert' ? 'تعيين' : 'حذف'} التنبيه...`;
        ...
        try {
            ...
            if (response.ok && data.status === 'success') {
                alertStatus.textContent = `${action === 'setAlert' ? 'تم تعيين' : 'تم حذف'} التنبيه بنجاح.`;
                alertStatus.style.color = 'green';
                loadUserAlertsDisplay(); // تحديث قائمة التنبيهات بعد كل عملية
                return true;
            } else {
                alertStatus.textContent = `فشل ${action === 'setAlert' ? 'تعيين' : 'حذف'} التنبيه: ${data.message || 'خطأ غير معروف.'}`;
    ...
    setAlertButton.addEventListener('click', async () => {
        ...
        if (!isTelegramAlert && !isBrowserAlert) {
            alertStatus.textContent = 'الرجاء اختيار نوع واحد على الأقل من التنبيهات (تيليجرام أو المتصفح).';
            alertStatus.style.color = 'red';
            return;
        }

        const targetPrice = parseFloat(targetPriceInput.value);
        const alertCondition = document.querySelector('input[name="alertCondition"]:checked').value;
        ...
        if (isTelegramAlert && !telegramChatId) {
            alertStatus.textContent = 'الرجاء إدخال معرّف دردشة تيليجرام لتنبيه تيليجرام.';
            alertStatus.style.color = 'red';
            return;
        }
        ...
        if (isTelegramAlert) {
            ...
            if (success) {
                alertStatus.textContent += (isBrowserAlert ? ' و' : '') + `تم تعيين تنبيه تيليجرام لـ ${selectedSymbol}.`;
                alertStatus.style.color = 'green';
        ...
        if (!isTelegramAlert && isBrowserAlert) { // إذا تم تعيين تنبيه متصفح فقط
            targetPriceInput.value = '';
        }
    });

    async function deleteAlert(alertId) {
        const success = await manageAlertOnAppsScript('deleteAlert', { id: alertId });
        if (success) {
