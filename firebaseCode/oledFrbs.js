...
exports.proxyRequest = onRequest(
	...
	async (req, res) => {
		...
		try {
			...
			let alrtTlgrm = rslt.data.filter(
				coin =>
					parseFloat(coin.priceChangePercent) > 20 &&
...
async function fltrCrpto(dts) {
	...
	function rplSmblFn(row1, row2) {
		rpl.push(row2);
		rplcSmbl[row1].push(rpl);
	}
	dt.forEach(el => {
		if (!allCpts[el[2]]) {
			allCpts[el[2]] = el[1];
  ...
  // rplcSmbl = [["BTC-USDT","BTCUSDT"],["ETH-USDT","ETHUSDT"]]
